...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    if (navigator.clipboard && typeof navigator.clipboard.writeText === 'function') {
      navigator.clipboard.writeText('jatingidh04@gmail.com')
        .then(() => setcopied(true))
  ...
  return (
    <div
      className={cn(
        "row-span-1 relative overflow-hidden rounded-3xl group/bento hover:shadow-xl transition duration-200 shadow-input dark:shadow-none justify-between flex flex-col space-y-4 border border-white-100 ",
        className
      )}
      style={{
    ...
    >
        ...
        <div>
            {id === 6 && (
                <BackgroundGradientAnimation>
                    {/*<div className="absolute z-60 flex items-center justify-center text-white-100
                    font-bold"/> */}
                </BackgroundGradientAnimation>
            )} 
        </div>
        <div className={cn(
            ...
            <div className="font-sans font-extrlight text-[#c1c2d3] text-sm md:text-xs lg:text-base z-10">
                {description}
            </div>
